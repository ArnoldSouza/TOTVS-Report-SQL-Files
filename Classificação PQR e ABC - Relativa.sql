...
(
	SELECT 
		...
		(
			...
			WHERE
				...
				(
					SD3010.D3_EMISSAO>=DATEADD(DAY,-365,GETDATE()) AND --EQUIVALENTE A UM ANO
					SD3010.D3_EMISSAO<=GETDATE()
				) AND
				SD3010.D3_COD=SB2010.B2_COD AND --FIXA POR CÓDIGO
				SD3010.D3_FILIAL + SD3010.D3_LOCAL = Z01010.Z01_CODGER --FIXA O ARMAZÉM
		) AS 'POPULARIDADE SAI',
	...
	FROM PROTHEUS.dbo.SB2010 SB2010 -- LINK DE SALDO EM ESTOQUE
		LEFT JOIN PROTHEUS.dbo.Z01010 Z01010 ON Z01010.Z01_CODGER = (SB2010.B2_FILIAL+SB2010.B2_LOCAL) -- CONEXÃO DE ARMAZÉM 
		LEFT JOIN PROTHEUS.dbo.SB1010 SB1010 ON SB2010.B2_COD=SB1010.B1_COD AND SB1010.D_E_L_E_T_ <> '*' -- CONEXÃO DE PRODUTOS
		LEFT JOIN PROTHEUS.dbo.SBM010 SBM010 ON SB1010.B1_GRUPO=SBM010.BM_GRUPO -- CONEXÃO DE GRUPOS DE PRODUTOS
	WHERE
		(SB2010.B2_QATU<>'0') AND -- SOMENTE COM SALDO
		(Z01010.Z01_MSBLQL = 2) AND -- SOMENTE ARMAZEM DESBLOQUEADO
		--SB1010.B1_MSBLQL='2' AND -- SOMENTE PRODUTOS DESBLOQUEADOS
		(SB2010.D_E_L_E_T_<>'*') AND  -- DESCONSIDERA DELETADOS
		Z01010.Z01_CODGER IN ('0201','0211','0221','0222','0225','0228','0231','0232','0238','0239','0240','0245','0740','0742','0743','0744','0745','0746','0747','0950','0963','0965','0975','0982','1003','1289','1290','1291','1292','1293','1294','1303','1304','1305','1395','1396','1397','0220','0235','0242','0243','0244','0246','0749','0959','1286','1287','1288','1306','1307')
		--Z01010.Z01_CODGER IN ('0201','0221','0222') -- TODO: retirar esta declaração
)
...
SELECT
	*,
    ...
    END AS 'CLASSE ABC',
	CASE
		WHEN SD.[POPULARIDADE SAI] >= 48 THEN 'P'
		WHEN SD.[POPULARIDADE SAI] >= 12 THEN 'Q'
		WHEN SD.[POPULARIDADE SAI] >= 4 THEN 'R-TRI'
		WHEN SD.[POPULARIDADE SAI] >= 2 THEN 'R-SEM'
		WHEN SD.[POPULARIDADE SAI] >= 1 THEN 'R-ANO'
		WHEN SD.[POPULARIDADE SAI] = 0 THEN 'S'
		ELSE 'ERRO'
	END AS 'CLASSE PQR'
FROM SALDOS AS SD --WHERE SD.[VALOR UPC] > 0