WITH DADOS_PC AS
	(
		SELECT
			...
			ISNULL(REPLACE(RTRIM(CTT010.CTT_XREGIO),'_',' '),
				CASE
				    ...
				    WHEN SC1010.C1_FILIAL = '07' AND (CASE WHEN SC7010.C7_XLOCENT='' THEN SC1010.C1_LOCAL ELSE SC7010.C7_XLOCENT END) = '56'	THEN 'AMPLA'
				    WHEN SC1010.C1_FILIAL = '02' AND (CASE WHEN SC7010.C7_XLOCENT='' THEN SC1010.C1_LOCAL ELSE SC7010.C7_XLOCENT END) = '50'	THEN 'CELPA NORDESTE'
				    WHEN SC1010.C1_FILIAL = '12' AND (CASE WHEN SC7010.C7_XLOCENT='' THEN SC1010.C1_LOCAL ELSE SC7010.C7_XLOCENT END) = '70'	THEN 'CELPA OESTE'
					ELSE 'ERRO'
				END
			) AS 'REGIONAL',
		...
		FROM PROTHEUS.dbo.SC7010 SC7010
			...
			LEFT MERGE JOIN PROTHEUS.dbo.CTT010 CTT010 ON SC7010.C7_CC=CTT010.CTT_CUSTO AND CTT010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.CT1010 CT1010 ON SC7010.C7_CONTA=CT1010.CT1_CONTA AND CT1010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.CTD010 CTD010 ON SC7010.C7_ITEMCTA=CTD010.CTD_ITEM AND CTD010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.CTH010 CTH010 ON SC7010.C7_CLVL=CTH010.CTH_CLVL AND CTH010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.SB1010 SB1010 ON SC7010.C7_PRODUTO=SB1010.B1_COD AND SB1010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.SBM010 SBM010 ON SB1010.B1_GRUPO=SBM010.BM_GRUPO AND SBM010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.SY1010 SY1010 ON SC7010.C7_USER=SY1010.Y1_USER AND SY1010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.SE4010 SE4010 ON SC7010.C7_COND=SE4010.E4_CODIGO AND SE4010.D_E_L_E_T_<>'*'
			LEFT MERGE JOIN PROTHEUS.dbo.SA2010 SA2010 ON SA2010.A2_COD=SC7010.C7_FORNECE AND SA2010.A2_LOJA=SC7010.C7_LOJA AND SA2010.D_E_L_E_T_<>'*'
			--CONEXÃO COM TABELA DE COMPRAS (SC1)
			LEFT MERGE JOIN PROTHEUS.dbo.SC1010 SC1010 ON ((SC1010.C1_NUM = SC7010.C7_NUMSC) AND (SC1010.C1_ITEM = SC7010.C7_ITEMSC)) AND SC1010.D_E_L_E_T_<>'*'
		WHERE 
			(SC7010.C7_EMISSAO>='20190101' AND SC7010.C7_EMISSAO<=GETDATE()) AND -- RESTRINGE INTERVALO DE TEMPO
...
-- 		 	(SC7010.C7_CONAPRO = 'L') AND --PC APROVADOS
		 	(SC7010.C7_QUJE+SC7010.C7_QTDACLA)<SC7010.C7_QUANT AND --SOMENTE MATERIAL PENDENTE
