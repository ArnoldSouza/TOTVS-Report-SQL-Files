...
SELECT 
	...
	) AS 'DATA_ATUALIZA' -- BUSCA A ULTIMA DATA DE ATUALIZAÇÃO DO PROCESSO (DATA DE APROVAÇÃO PARA SC APROVADAS)
...
SELECT
	...
	ISNULL((
		...
		FROM
		(VALUES (SC1010.C1_DTLIB1),(SC1010.C1_DTLIB2),(SC1010.C1_DTLIB3),(SC1010.C1_DTLIB4),(SC1010.C1_DTLIB5),(SC1010.C1_DTLIB6),(SC1010.C1_DTLIB7),(SC1010.C1_EMISSAO),(SC1010.C1_ALTDATA)) AS UpdateDate(LastUpdateDate)
	), SF1010.F1_DTDIGIT) AS 'DATA_ATUALIZA' -- BUSCA A ULTIMA DATA DE ATUALIZAÇÃO DO PROCESSO (DATA DE APROVAÇÃO PARA SC APROVADAS)
...
SELECT
	...
	'TRANF. INTERNA' AS 'PROCESSO',
	(
		SELECT MAX(LastUpdateDate)
		FROM
		(VALUES (Z22010.Z22_DTLIB1),(Z22010.Z22_DTLIB2),(Z22010.Z22_DTLIB3),(Z22010.Z22_DTLIB4),(Z22010.Z22_DTLIB5),(Z22010.Z22_DTLIB6),(Z22010.Z22_DTLIB7),(Z22010.Z22_EMISSA)) AS UpdateDate(LastUpdateDate)
	) AS 'DATA_ATUALIZA' -- BUSCA A ULTIMA DATA DE ATUALIZAÇÃO DO PROCESSO (DATA DE APROVAÇÃO PARA SC APROVADAS)
...
WHERE
	...
	AND Z22010.Z22_DESEMB <>'1'
	AND(Z22010.Z22_EMISSA>=DATEADD(DAY,-365,GETDATE()) AND Z22010.Z22_EMISSA<=GETDATE())
)
/* RESUMO ANALÍTICO */
SELECT
	*,
	DATEDIFF(DAY, PIP.DATA_ATUALIZA, GETDATE()) AS TEMPO_ESPERA
...
-- SELECT
	...
	-- MIN(DATEDIFF(DAY, PIP.DATA_ATUALIZA, GETDATE())) AS TEMPO_ESPERA
-- FROM PIPELINE AS PIP
--WHERE PIP.FILIAL = '02' AND PIP.LOCAL = '25' -- EXEMPLO FIXANDO ARMAZÉM
-- GROUP BY 
	-- PIP.FILIAL,
