SELECT
	...
	CASE
        WHEN CONCAT(SC7010.C7_FILENT,SC7010.C7_LOCAL) IN ('0240','0245')
    ...
    --DEFINIÇÃO DE REGIONAIS DE ACORDO COM CENTRO DE CUSTO
    CASE
        --DEFINIÇÃO CELPA GERAL
        WHEN SC7010.C7_CC='0072505' THEN 'CELPA GERAL'
        --RESQUICIOS DO CONTRATO COELBA A7
        WHEN
            (
                SC7010.C7_CC LIKE '01994%' OR
                SC7010.C7_CC LIKE '01995%' OR
                SC7010.C7_CC LIKE '01996%' OR
                SC7010.C7_CC LIKE '01997%'
            ) THEN 'COELBA A7'
        --DEFINICAO RIO DE JANEIRO
        WHEN SC7010.C7_CC LIKE '00306%' THEN 'ENEL GERAL'
        WHEN SC7010.C7_CC LIKE '00307%' THEN 'ENEL LAGOS'
        WHEN SC7010.C7_CC LIKE '00308%' THEN 'ENEL LAGOS'
        WHEN SC7010.C7_CC LIKE '00309%' THEN 'ENEL LAGOS'
        WHEN SC7010.C7_CC LIKE '00389%' THEN 'ENEL LAGOS'
        WHEN SC7010.C7_CC LIKE '00390%' THEN 'ENEL LAGOS'
        WHEN SC7010.C7_CC LIKE '00310%' THEN 'ENEL SERRA'
        --DEFINICAO NORMAL DE REGIONAIS
        ELSE ISNULL(REPLACE(RTRIM(CTT010.CTT_XREGIO),'_',' '), 'ESTOQUE')
    END AS 'REGIONAL CC',

    -- --CAMPO DE REGIONAL CONSIDERANDO APONTAMENTOS EM CENTRO DE CUSTO
    -- ISNULL(REPLACE(RTRIM(CTT010.CTT_XREGIO),'_',' '), 'ESTOQUE') AS 'REGIONAL CC',
    ...
    CASE
        WHEN SC7010.C7_TPCOMPR=3 THEN 'ATIVO'
        WHEN SC7010.C7_TPCOMPR=2 THEN 'AP DIRETA'
        WHEN SC7010.C7_TPCOMPR=1 THEN 'ESTOQUE'
        ELSE 'ERRO'
    END AS 'TIPO DE COMPRA DECLARADO',
    CASE
        WHEN LEFT(SC7010.C7_CONTA,3)='132' OR SC7010.C7_CONTA='' OR LEFT(BM_GRUPO,1)='4'
            THEN 'ATIVO'
        WHEN SC7010.C7_CC<>'' THEN 'AP DIRETA'
        WHEN SC7010.C7_CC='' THEN 'ESTOQUE'
        ELSE 'ERRO'
    END AS 'TIPO DE COMPRA REAL',
    ...
    --INFO RETIRADA DO CABECALHO DA NOTA FISCAL DE ENTRADA
...
FROM PROTHEUS.dbo.SC7010 SC7010 WITH (NOLOCK)
    ...
    LEFT JOIN PROTHEUS.dbo.CTT010 CTT010 WITH (NOLOCK) ON SC7010.C7_CC=CTT010.CTT_CUSTO AND CTT010.D_E_L_E_T_<>'*'
...
WHERE
    (SC7010.C7_EMISSAO>=CONCAT(YEAR(GETDATE())-1,'0101') AND SC7010.C7_EMISSAO<=GETDATE()) AND -- RESTRINGE INTERVALO DE TEMPO PARA O ANO ANTERIOR ATÉ HOJE
...
--TODO: FALTA ADICIONAR QUANTO TEMPO ATÉ CHEGAR O MATERIAL (COMPARAR SC7 COM SD1), UTILIZAR A DATA DE RECEBIMENTO
--TODO: RETIRAR TODOS OS 'TOTAIS' DA BASE. SAO CAMPOS CALCULADOS NAO NECESSARIOS NA TABELA FATO

-- >>> OBSERVAÇÕES <<< --
--O CAMPO 'TEMPO ATÉ COMPRAR' É UM CAMPO CALCULADO, NAO NECESSÁRIO CRIAR NO SQL
--SE A FILIAL É 0240 OU 0245 ENTAO DEVE-SE CONSIDERAR A FILIAL DE DESTINO PARA SE CHEGAR A REGIONAL CORRETA DE APROPRIAÇÃO DO MATERIAL